...
alter table tblsched add constraint tblsched_scanner_fkey foreign key (scannercode) references tlkpscanner(scannercode) on update cascade deferrable;
...
ALTER TABLE ONLY tbltemplate
    ADD CONSTRAINT tbltemplate_scanner_and_templatecode_fkey FOREIGN KEY (scannercode, templatecode) REFERENCES tbltemplates(scannercode, templatecode) ON UPDATE CASCADE DEFERRABLE;


alter table tbltemplate add constraint tbltemplate_scannercode_fkey foreign key (scannercode) references tlkpscanner(scannercode) on update cascade deferrable;
