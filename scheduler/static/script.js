...
import {
  enableBodyScroll,
  disableBodyScroll,
} from "./scroll-lock/body-scroll-lock.js";
...
function enforce_datalists() {
  ...
  const mo = new MutationObserver((ms) => {
    for (const m of ms) {
      const t = m.target;
      if (t.list != null) {
        // can be null when between lists
        t.setCustomValidity("");
...
async function confirmDialog(titleText, message, opts = {}) {
  return new Promise((resolve, _) => {
    ...
    dialog.on("hide", (_, evt) => {
      ...
      resolve(
        Boolean(evt.target && evt.target.name && evt.target.name == "confirm"),
      );
      // reset template
      title.innerText = "";
      body.innerHTML = "";
...
async function loadLog(id, template, signal) {
  try {
    const resp = await fetch(`${base}json/v1/log/${id}-${template}`, {
      signal,
    });
    if (!resp.ok) {
...
function wire_log_buttons() {
  ...
  editor
    .querySelectorAll("entry-cell button[data-for]")
...
function editor_grid() {
  ...
  editor.querySelector("tbody").addEventListener("click", (evt) => {
    let t = evt.target;
    const setsInternalFocus = t.matches(
      ":is(input, button, label):not(:disabled)",
    );
  ...
  container.addEventListener("keydown", (evt) => {
    ...
    switch (evt.key) {
      ...
      case "Enter":
        ...
        grid
          .selectedCell()
          .querySelector(":is(input, button):not(:disabled)")
          .focus();
        break;
...
class SupportRequestSubForm {
  ...
  get changed() {
    ...
    if (
      ...
      this.subkind != null &&
      this.orig.subkind != this.subkind.value
    ) {
      return true;
    }
...
class Cell {
  ...
  get changed() {
    ...
    return (
      this.orig.group != this.group.value ||
      this.orig.member != this.member.value
    );
  }
  get invalid() {
    if (!this.editable) {
  ...
  update_swatch() {
    this.legend.style.setProperty(
      "--swatch",
      this.colors[this.group.value] ?? "transparent",
    );
  }
  fire_change() {
    this.elm.dispatchEvent(
      new CustomEvent("cell_change", {
        bubbles: true,
        detail: this,
      }),
    );
...
function fmt_editor_diffs(human_hours, templateEditor, diffs) {
  ...
  for (const diff of diffs) {
    acc.push(
      `<dt>${humanize_date_if_needed(diff.date)}: ${
        human_hours[diff.hour]
      }</dt>`,
    );
...
function fmt_save_results(data, cells, human_hours) {
  ...
  if (errors.length > 1) {
    acc.push(
      `${errors.length} irreconcilable errors. Affected entries will be reset and locked, `,
    );
  } else {
    acc.push(
      "Irreconcilable error. The affected entry will be reset and locked, ",
    );
  }
    ...
    for (const err of errs) {
      ...
      switch (err.type) {
        ...
        case "overwrote":
          ...
          const got = err.got;
          acc.push(
            `${key} overwritten by another user: expected the old value to be `,
          );
          if (expected) {
...
function wire_cell_editors() {
  ...
  const templateEditor = document
    .querySelector("form.editor")
    .classList.contains("template-editor");
  ...
  const schedulable_groups = new Set(
    [...document.querySelectorAll("datalist[id^=members-of-]")].map((elm) =>
      elm.id.slice("members-of-".length),
    ),
  );
  ...
  const cells = new Map(
    [...container.querySelectorAll("td entry-cell")].map((elm) => {
  ...
  apply.addEventListener("click", (evt) => {
    ...
    if (!any_changed()) {
      ...
      editor_dialog(
        ...
        false,
        "outstanding errors",
        "all errors must be resolved in updated entries before submitting",
        null,
      );
    } else {
      ...
      let [notifications, summary] = fmt_editor_diffs(
        human_hours,
        templateEditor,
        diffs,
      );
