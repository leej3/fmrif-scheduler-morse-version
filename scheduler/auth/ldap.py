...
class LDAPClient:
    ...
    def authenticate(self, username: str, password: str) -> Tuple[bool, Optional[LDAPUser]]:
        ...
        try:
            ...
            current_app.logger.info(f"[AUTH] LDAP Server: {self.config.host}:{self.config.port}")
            ...
            if '@' in username:
                ...
                current_app.logger.info("[CONN] Creating server connection for email lookup")
                server = ldap3.Server(
                    self.config.host,
                    port=self.config.port,
                    use_ssl=self.config.use_ssl,
                ...
                if server.info:
                    ...
                    current_app.logger.info(f"[SERVER] Version: {server.info.vendor_version}")
                ...
                search_filter = f"(&(objectClass={self.config.user_object_class})(mail={email}))"
                current_app.logger.info(f"[SEARCH] Email search filter: {search_filter}")
            ...
            if not conn.entries:
                current_app.logger.warning(f"[SEARCH] No user found with sAMAccountName: {username}")
                return False, None
            ...
            current_app.logger.info(f"[USER] DN: {user_entry.distinguishedName or 'Not Set'}")
            current_app.logger.info(f"[USER] Display Name: {user_entry.displayName or 'Not Set'}")
            current_app.logger.info(f"[USER] Email: {user_entry.mail or 'Not Set'}")
            current_app.logger.info(f"[USER] Department: {user_entry.department or 'Not Set'}")
            ...
            if hasattr(user_entry, 'msDS-UserAccountDisabled'):
                ...
                current_app.logger.info(f"[STATUS] msDS-UserAccountDisabled: {disabled_status}")
                if disabled_status:
                    current_app.logger.warning("[STATUS] Account is explicitly disabled via msDS-UserAccountDisabled")
            else:
                current_app.logger.info("[STATUS] msDS-UserAccountDisabled attribute not present")
            ...
            try:
                if hasattr(user_entry, 'userAccountControl') and user_entry.userAccountControl:
                    ...
                    current_app.logger.info(f"[STATUS] User Account Control (Raw Value): {uac}")
                    ...
                    for flag_value, flag_name in uac_flags.items():
                        if uac & flag_value:
                            current_app.logger.info(f"[STATUS] Active Flag: {flag_name} (0x{flag_value:x})")
                            if flag_name == 'ACCOUNTDISABLE':
                                current_app.logger.warning("[STATUS] Account is disabled via UAC flag")
                    ...
                    current_app.logger.warning("[STATUS] No userAccountControl attribute available - cannot determine account status flags")
            except (ValueError, TypeError) as e:
                current_app.logger.error(f"[STATUS] Error processing userAccountControl: {str(e)}")
            ...
            windows_to_unix_epoch = 11644473600  # Seconds between Windows and Unix epochs
            ...
            for attr, desc in timestamp_attrs.items():
                if hasattr(user_entry, attr) and getattr(user_entry, attr):
                    ...
                    current_app.logger.info(f"[STATUS] {desc}: {human_time} (Raw: {timestamp})")
                else:
            ...
            if hasattr(user_entry, 'lockoutTime') and user_entry.lockoutTime:
                ...
                if lockout_time > 0:
                    ...
                    current_app.logger.warning(f"[STATUS] Account is locked out since: {human_time}")
                else:
            ...
            if hasattr(user_entry, 'badPwdCount'):
                current_app.logger.info(f"[STATUS] Bad password attempts: {user_entry.badPwdCount.value}")
            ...
            current_app.logger.info(f"[BIND] DN format check: starts with 'CN=' - {user_dn.startswith('CN=')}")
            current_app.logger.info(f"[BIND] DN format check: contains ',OU=Users,DC=nih,DC=gov' - {',OU=Users,DC=nih,DC=gov' in user_dn}")
            ...
            try:
                ...
                current_app.logger.info(f"[BIND] Connection strategy: {auth_conn.strategy}")
                current_app.logger.info(f"[BIND] Authentication type: {auth_conn.authentication}")
                ...
                if not bind_result:
                    ...
                    data = error_msg.split('data ')[-1].split(',')[0] if 'data' in error_msg else 'unknown'
                    ...
                    error_message = error_codes.get(data, 'Unknown authentication error')
                ...
                if current_app.debug and current_app.config.get('LDAP_DEBUG_TRACE', False):
                    current_app.logger.debug(f"[AUTH] Debug trace:\n{traceback.format_exc()}")
                return False, None
            ...
            current_app.logger.info(f"[SUCCESS] Authentication successful for user: {user.username}")
            current_app.logger.info("="*50)
            return True, user
