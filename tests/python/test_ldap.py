...
@pytest.mark.ldap
@pytest.mark.skipif(not can_connect_to_ldap(), reason="LDAP server not reachable - Are you on NIH network/VPN?")
class TestLDAPConnection:
    ...
    def test_user_search(self, ldap_connection, test_user):
        ...
        try:
            ...
            assert len(ldap_connection.entries) == 1, f"Expected 1 result, got {len(ldap_connection.entries)}"
            ...
            for attr in settings.ldap.required_user_attrs:
                assert hasattr(user, attr), f"Required attribute {attr} missing from user data"
            ...
            assert hasattr(user, 'msDS-UserAccountDisabled'), "Account status attribute missing"
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "ldap: mark test as requiring LDAP connectivity"
    ) 